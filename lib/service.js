...
async function callWhisper(data, runWhisperLocally) {
  return new Promise(async (resolve, reject) => {
      ...
      // Try to retrieve existing transcript if we have a GUID
      if(data.guid){
        try {
          ...
          if (existingTranscript && existingTranscript?.cdnFileId) {
              try {
                ...
                // Define transcriptData in the correct scope
                let transcriptData = null;
                
                try {
                  transcriptData = await fetchJsonData('cascdr-transcripts', DOFileId);
                } catch (fetchErr) {
                  console.error(`Error during fetchJsonData: ${fetchErr.message}`);
                  // Continue with transcription as if no existing transcript was found
                }
                
                // Check if transcriptData is null (indicating retrieval failure)
                if (transcriptData === null) {
                  console.log(`Failed to fetch cached transcript for guid:${data.guid}, will re-transcribe.`);
                  ...
                  try {
                    ...
                    console.error(`Error logging transcript data: ${logErr.message}`);
                    // Continue even if there's a logging error
                  }
                  ...
                  if (typeof transcriptData === 'string') {
                      try {
                          ...
                          console.log("About to resolve with transcriptData:", parsedData);
                          resolve(parsedData);
                      ...
                      // Additional safety check for non-null object
                      if (transcriptData !== null) {
      ...
      // Return early if we found and returned an existing transcript
      if (!shouldTranscribe) {
        return;
      }
