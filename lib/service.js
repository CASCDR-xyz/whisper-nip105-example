  ...
  return new Promise(async (resolve, reject) => {
      ...
      if(data.guid){
        try {
              ...
              if (transcriptData) {
                ...
                if (typeof transcriptData === 'string') {
                    try {
                        ...
                        // Continue with transcription if parsing fails
                    }
                }
          ...
          console.log(`Error while retrieving existing transcript: ${e.message}`);
          // Continue with transcription if retrieval fails
        }
        
        // If we didn't find or couldn't fetch the transcript, log it and continue
        console.log("Did not find valid existingTranscript for guid:", data.guid);
      }
      
      // Continue with new transcription
      const audioFilePath = data.filePath;
      console.log(`Full audio file path: ${audioFilePath}`);
      console.log(`callWhisper data:${JSON.stringify(data, null, 2)}`);
      ...
      if (runWhisperLocally) {
          ...
          try {
            ...
            // Cache the transcript if it doesn't exist and guid is provided
            if(!existingTranscript && data.guid){
              try {
                ...
                console.log(`Successfully cached transcript for guid: ${data.guid}`);
              } catch (cacheError) {
                console.error(`Error caching transcript: ${cacheError.message}`);
                // We already resolved with the result, so just log the error
              }
            }
          } catch (error) {
