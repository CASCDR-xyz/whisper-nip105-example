...
const FILE_SIZE_LIMIT_MB = parseInt(process.env.FILE_SIZE_LIMIT_MB || "500", 10);
...
async function getRemoteFileSize(remoteUrl) {
  try {
    ...
    if (!contentLength) {
      console.warn('Content-Length header not available for remote file');
      return null;
    }
